...
test.describe.serial("inbox ingestion smoke", () => {
  test("imports pasted advice, reviews claims, and sends approved claims to Atlas", async ({
    ...
    await page.getByRole("link", { name: "Review claims" }).first().click();
    await expect(page.getByRole("heading", { name: "Claims" })).toBeVisible();

    await page
      .getByLabel("Canonical claim")
      .first()
      .fill("Ask for recommendation letters while the professor still remembers your work.");
    ...
    await expect(page.getByRole("button", { name: /Send \d+ approved claims to Atlas/ })).toBeEnabled();
    await page.getByRole("button", { name: /Send \d+ approved claims to Atlas/ }).click();
